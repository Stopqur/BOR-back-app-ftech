...
exports.createRecipe = async (req, res) => {
  try {
    if (req.file === undefined) {
      ...
      cookingSteps.map(async (item) => {
        await db.cooking_steps.create({
          name: item.name,
          recipe_id: recipe.id,
        });
      });
...
exports.deleteRecipe = async (req, res) => {
  try {
    ...
    const steps = await db.cooking_steps.findAll({
    ...
    recipe.destroy();
    const userRecipes = await db.recipes.findAll({
      include: [db.cooking_steps.name],
    });
...
exports.createWishRecipe = async (req, res) => {
  try {
    ...
    if (wishRecipe === undefined) {
      const addedRecipe = await db.users_recipes.findOne({
